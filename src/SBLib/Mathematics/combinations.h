...
struct select_combinations
{
	...
	{
		static_assert(rank_size <= dimension_size, "Invalid rank");
		...
		enum : size_t
		{
			...
			value_from_conjugate = space_mask & ~select_combinations<space_mask, conjugate_rank_size>::template get<conjugate_index>(),
			...
			inherited_value      = select_combinations<inherited_space_mask, inherited_rank_size>::template get<inherited_index>(),

			constructed_index    = is_constructed ? constructed_count - (count - index - 1) - 1 : 0,
			constructed_value    = last_bit | select_combinations<inherited_space_mask, rank_size - 1>::template get<constructed_index>(),
	...
	enum : size_t
	{
		value = select_combinations::template get<index>(),
	};
	...
	constexpr static size_t get_components_index()
	{
		...
		enum : size_t
		{
			...
			value_from_conjugate = select_combinations<space_mask, conjugate_rank_size>::template get_components_index<conjugate_mask>(),

			inherited_rank       = is_inherited_index ? inherited_rank_size : 0,
			inherited_mask       = is_inherited_index ? subspace_mask : 0,
			inherited_raw_value  = select_combinations<inherited_space_mask, inherited_rank>::template get_components_index<inherited_mask>(),
			...
			constructed_value    = inherited_count + select_combinations<inherited_space_mask, constructed_rank>::template get_components_index<constructed_submask>(),
...
template<size_t space_mask, size_t index>
struct select_combinations<space_mask, 0, index>
{
	enum : size_t
	{
		space_mask     = space_mask,
...
template<size_t space_mask, size_t index>
struct select_combinations<space_mask, 1, index>
{
	enum : size_t
	{
		space_mask     = space_mask,
